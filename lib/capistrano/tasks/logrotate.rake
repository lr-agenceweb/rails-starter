...
namespace :logrotate do
  ...
  task :upload_config do
    on roles(:web) do
      ...
      logrotate_file_tmp = "/tmp/logrotate_#{Figaro.env.application_name.underscore}_#{fetch(:stage)}.conf"
      logrotate_file_dest = "/etc/logrotate.d/#{Figaro.env.application_name.underscore}_#{fetch(:stage)}.conf"
