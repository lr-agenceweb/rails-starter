...
module Admin
  ...
  class UsersControllerTest < ActionController::TestCase
    ...
    test 'should return correct value for toggle_active batch action' do
      post :batch_action, params: { batch_action: 'toggle_active', collection_selection: [@administrator_two.id, @bart.id] }
      [@administrator_two, @bart].each(&:reload)
      assert_not @administrator_two.account_active?
      assert @bart.account_active?
    end

    test 'should not toggle_active current user in batch action' do
      post :batch_action, params: { batch_action: 'toggle_active', collection_selection: [@administrator.id] }
    ...
    test 'should not toggle_active super_administrator by administrator in batch action' do
      post :batch_action, params: { batch_action: 'toggle_active', collection_selection: [@super_administrator.id] }
    ...
    test 'should redirect to back and have correct flash notice for toggle_active batch action' do
      post :batch_action, params: { batch_action: 'toggle_active', collection_selection: [@administrator_two.id] }
      assert_redirected_to admin_users_path
    ...
    test 'should redirect to back and have correct flash notice for reset_cache batch action' do
      post :batch_action, params: { batch_action: 'reset_cache', collection_selection: [@bart.id] }
      assert_redirected_to admin_users_path
    ...
    test 'should be able to create user' do
      ...
      attrs = { username: 'Marco', email: 'marco@test.com', password: 'password', confirm_password: 'password' }
    ...
    test 'should not be able to create if username already taken' do
      sign_in @super_administrator
      attrs = { username: 'bob', email: 'bob@test.fr', password: 'password', confirm_password: 'password' }
    ...
    test 'should not be able to update role_id in SA if admin' do
      patch :update, params: { id: @administrator, user: { role_id: @super_administrator.role_id } }
    ...
    test 'should not be able to update role_id if wrong id and admin' do
      patch :update, params: { id: @administrator, user: { role_id: '778899' } }
      assert_equal assigns(:user).role_id, @administrator.role_id
      assert_equal assigns(:user).role_name, @administrator.role_name
    end
    ...
    test 'should be able to update role_id if user is admin' do
      patch :update, params: { id: @administrator, user: { role_id: @subscriber.role_id } }
      assert_equal @subscriber.role_id, assigns(:user).role_id
      assert_equal @subscriber.role_name, assigns(:user).role_name
    end
    ...
    test 'should not be able to update role_id if user is subscriber' do
      sign_in @subscriber
      patch :update, params: { id: @subscriber, user: { role_id: @super_administrator.role_id } }
    ...
    test 'should test abilities for administrator' do
      ...
      assert ability.can?(:read, @administrator_two), 'should be able to read other administrator'
      assert ability.cannot?(:read, @super_administrator), 'should not be able to read super_administrator'
      ...
      assert ability.cannot?(:update, @administrator_two), 'should not be able to update other administrator'
      assert ability.cannot?(:update, @super_administrator), 'should not be able to update super_administrator'
      ...
      assert ability.cannot?(:destroy, @administrator_two), 'should not be able to destroy other administrator'
      assert ability.cannot?(:destroy, @super_administrator), 'should not be able to destroy super_administrator'
      ...
      assert ability.cannot?(:toggle_active, @administrator_two), 'should not be able to toggle_active other administrator'
      assert ability.cannot?(:toggle_active, @super_administrator), 'should not be able to toggle_active super_administrator'
    end
    ...
    test 'should test abilities for super_administrator' do
      ...
      assert ability.can?(:read, @super_administrator_two), 'should be able to read other super_administrator'

      assert ability.can?(:update, @subscriber), 'should be able to update subscriber'
      assert ability.can?(:update, User.new), 'should be able to update'
      ...
      assert ability.cannot?(:update, @super_administrator_two), 'should not be able to update other super_administrator'

      assert ability.can?(:destroy, @subscriber), 'should be able to destroy subscriber'
      assert ability.can?(:destroy, User.new), 'should be able to destroy'
      ...
      assert ability.cannot?(:destroy, @super_administrator_two), 'should not be able to destroy other super_administrator'
      ...
      assert ability.cannot?(:toggle_active, @super_administrator_two), 'should not be able to toggle_active other super_administrator'
    end
    ...
    def initialize_test
      ...
      @administrator_two = users(:lorie)
      @super_administrator = users(:anthony)
      @super_administrator_two = users(:maria)
    ...
    def upload_paperclip_attachment
      ...
      patch :update, params: { id: @administrator, user: { avatar: attachment } }
    end

    def remove_paperclip_attachment(user)
      # puts '=== Removing avatar'
      patch :update, params: { id: user, user: { avatar: nil, delete_avatar: '1' } }
